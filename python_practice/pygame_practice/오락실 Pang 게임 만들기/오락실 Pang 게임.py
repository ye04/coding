...
pygame.display.set_caption("오락실 Pang 게임")
...
stage_height = stage_size[1] #스테이지를 기준으로 캐릭터를 두기 위해 사용
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls): #리스트 아이템의 인덱스와 그 값을 함께 가져옴
    ...
    for ball_idx, ball_val in enumerate(balls): #리스트 아이템의 인덱스와 그 값을 함께 가져옴
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remove = weapon_idx #해당하는 무기의 인덱스를 weapon_to_remove에 저장
    ...
    if ball_to_remove > -1:
        del balls[ball_to_remove] #리스트에서 그 값 삭제 --> 화면에 그릴 값 삭제 --> 화면에서 삭제
        ball_to_remove = -1
